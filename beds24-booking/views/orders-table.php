...
//region Test
//dd(@json_decode(get_post_meta(5104, 'request_api_res', true)));

/*if(!class_exists('\beds_booking\Action_beds_booking')){
    require(BEDS_DIR . '/includes/class.action.php');
}
$actionBedsBooking = new \beds_booking\Action_beds_booking();
$actionBedsBooking->setBookingOnAPI(5104);*/

/*$str = '[{"roomId":"446390","status":"confirmed","arrival":"2025-08-03","departure":"2025-08-10","numAdult":"1","numChild":"0","firstName":"Martin","lastName":"Rehnberg","email":"martin_rehnberg@hotmail.com","phone":"+46467617683","address":"Drottninggatan 40H","city":"V\u00e4ster\u00e5s","state":"","postcode":"72464","country2":"SE","invoiceItems":[{"type":"charge","qty":1,"amount":4500,"lineTotal":4500,"description":"[snow-cave-test] [2025-08-03] - [2025-08-10]"},{"type":"charge","qty":1,"amount":0,"lineTotal":0,"description":"Avresest\u00e4dning"},{"type":"payment","qty":1,"amount":4500,"lineTotal":"-4500","description":"[Swish payment 2025-02-25]"}],"flagColor":"FFFF00","flagText":"Departure cleaning"}]';
dd(json_decode($str));*/
...
$pricetable_settings = get_option('beds24_pricelis_table_settings');
...
<style>
    ...
    .orders-table thead th.sort::after {
        content: '▲';
        position: absolute;
        right: 5px;
        font-size: 12px;
    ...
    .orders-table thead th.sort.desc::after {
        content: '▼';
    }

    .orders-table thead th.sort.asc::after {
        content: '▲';
    }
...
<div class="orders-table-wrap">
    <div class="orders-table-filter">
        <button type="button" class="btn btn-transparent" data-filter="cleaning">Departure cleanings</button>
        <button type="button" class="btn btn-transparent" data-filter="past">View past</button>
    </div>
    <table class="orders-table">
        ...
        <?php foreach($orders as $order):
            foreach($order->get_items() AS $item):
                if(!$item->get_meta('booked_from') || !$item->get_meta('booked_to')) continue;
                $week = $weeks_data = $wpdb->get_row("SELECT * FROM beds_pricelist_weeks where start_date <= '".$wpdb->_real_escape($item->get_meta('booked_from'))."' and end_date >= '".$wpdb->_real_escape($item->get_meta('booked_to'))."'");
                $week_num = '';
                if($week){
                    $week_name_arr = explode(' ', $week->week_name);
                    if(count($week_name_arr) == 2){
                        $week_num = $week_name_arr[1];
                    }
                    else{
                        $week_num = $week->week_name;
                    }
                }
                $date_start = DateTime::createFromFormat('Y-m-d', $item->get_meta('booked_from'));
                $date_end = DateTime::createFromFormat('Y-m-d', $item->get_meta('booked_to'));
                $today = new DateTime();
                $payments = array_map(function($p){
                    return wc_get_order($p->ID);
                }, $wpdb->get_results("select * from $wpdb->posts where post_type = 'awcdp_payment' and post_parent = ".$order->get_id()." order by ID ASC"));

                $date_start_text = $date_start->format('Y-m-d').' ('.$date_start->format('D').')';
                $date_end_text = $date_end->format('Y-m-d').' ('.$date_end->format('D').')';

                if(!$week_num){
                    $date_start_by_week = clone $date_start;
                    $breakpoint = 0;
                    while($date_start_by_week->format('N') != 7){
                        $date_start_by_week->modify('-1day');
                        $breakpoint++;
                        if($breakpoint > 10) die('While error');
                    }
                    $date_start_text = $date_start_by_week->format('Y-m-d').' ('.$date_start_by_week->format('D').')';
                    $date_end_by_week = clone $date_end;
                    $breakpoint = 0;
                    while($date_end_by_week->format('N') != 7){
                        $date_end_by_week->modify('+1day');
                        $breakpoint++;
                        if($breakpoint > 10) die('While error');
                    }
                    $date_end_text = $date_end_by_week->format('Y-m-d').' ('.$date_end_by_week->format('D').')';
                }
                $tr_priority = 1000;
                if($item->get_product()){
                    $item_tags_ids = $item->get_product()->get_tag_ids();
                    if($item_tags_ids){
                        $item_tag_id = reset($item_tags_ids);
                        if(!empty($pricetable_settings['product_tag_order'][$item_tag_id])){
                            $tr_priority = $pricetable_settings['product_tag_order'][$item_tag_id];
                        }
                    }
                }

                $is_past = (int)($date_start->format('Y-m-d') < $today->format('Y-m-d'));

                ?>
                <tr data-priority="<?= $tr_priority ?>" data-cleaning="<?= (int)(!!$item->get_meta(__( 'Avresestädning'))) ?>" data-past="<?= $is_past ?>" style="<?= $is_past ? 'display: none;' : '' ?>">
                    <td>
                        <?= $week_num ?: $date_end->format('W') ?>
                    </td>
                    <td>
                        <?= $date_start_text ?>
                    </td>
                    ...
                    <td>
                        <a href="<?= $order->get_edit_order_url() ?>" target="_blank"><?= $order->get_id() ?></a>
                    </td>
                    <td>
                        <?= wc_price($item->get_total()+$item->get_total_tax()) ?>
                    </td>
                    <?php
                    $payment = $payments[0] ?? null;
                    $td_classes = [];
                    if(!$order->get_payment_method() && (!$payment || $payment->get_status() != 'completed')){
                        $td_classes[] = 'bg-red';
                    }
                    $d = clone $order->get_date_created();
                    $d->modify('+3 days');
                    $is_payed = ($payment && $payment->get_status() == 'completed') || ($order->get_payment_method() && $order->get_status() == 'completed');
                    ?>
                    <td data-sort_value="<?= $is_payed ? 'ignore' : $d->format('Y-m-d H:i:s') ?>" class="<?= implode(' ', $td_classes) ?>">
                        <?php
                        if($is_payed){
                            echo wc_price($payment ? $payment->get_total() : $order->get_total());
                        }
                        else{
                            echo $d->format('Y-m-d');
                        }
                    ...
                    $days = $interval->days;
                    if(!$order->get_payment_method() && (!$payment || $payment->get_status() != 'completed')){
                        if($interval->invert || $days <= 7){
                            $td_classes[] = 'bg-red';
                        }
                        elseif ($days <= 30){
                            $td_classes[] = 'bg-yellow';
                        }
                    }
                    $is_payed = $order->get_payment_method() || ($payment && $payment->get_status() == 'completed');
                    ?>
                    <td data-sort_value="<?= $is_payed ? 'ignore' : $d->format('Y-m-d H:i:s') ?>" class="<?= implode(' ', $td_classes) ?>">
                        <?php
                        ...
                        elseif($is_payed) {
                            echo wc_price($payment->get_total());
                        }
                        else{
                            echo $d->format('Y-m-d');
                        }
                    ...
                    <td>
                        <?= $item->get_meta(__( 'Avresestädning')) ?: '-' ?>
                        <?php if($item->get_meta(_( 'Avresestädning with RUT'))): ?>
                            (RUT)
                        <?php endif; ?>
                    </td>
                    <?php
                    ...
                    //$selected_option = $item->get_meta('_table_assign');
                    $selected_option = get_post_meta($order->get_id(), '_item_'.$item->get_id().'_table_assign', true);
                    ?>
                    <td data-sort_value="<?= esc_attr($selected_option) ?>">
                        <select class="table_assign" data-order_id="<?= $order->get_id() ?>" data-item_id="<?= $item->get_id() ?>" style="border: none;">
                            <option value="">None</option>
                            <?php foreach($select_options_groups AS $group): ?>
                                <optgroup label="<?= esc_attr($group['group']) ?>">
                                    <?php foreach($group['options'] AS $v => $l): ?>
                                        <option value="<?= esc_attr($v) ?>" <?= $v == $selected_option ? 'selected' : '' ?>><?= $l ?></option>
                                    <?php endforeach; ?>
                                </optgroup>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <td>
                        <div class="info-block" style="display: flex;">
                            <button type="button" class="show-info-block" style="background-color: white; border: none;">Show</button>
                            <div class="info-block-content" style="display: none">
                                <p>
                                    <strong><?= __('Datum', 'beds24') ?></strong>
                                    <br>
                                    <span><?= $item->get_meta('booked_from').' - '.$item->get_meta('booked_to') ?></span>
                                </p>
                                <p>
                                    <strong><?= __('Boende', 'beds24') ?></strong>
                                    <br>
                                    <span><?= $item->get_name() ?></span>
                                </p>
                                <p>
                                    <strong><?= __('Gäst', 'beds24') ?></strong>
                                    <br>
                                    <span>
                                        <?= $order->get_billing_first_name().' '.$order->get_billing_last_name() ?>
                                        <br>
                                        ...
                                        <br>
                                        <?= $order->get_meta('_billing_ssn') ?>
                                    </span>
                                </p>
                                <a href="javascript:;" class="copy" style="position: absolute;bottom: 10px;right: 10px;">
                                    <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M3.95837 11.8747H3.16671C2.74678 11.8747 2.34405 11.7079 2.04712 11.4109C1.75019 11.114 1.58337 10.7113 1.58337 10.2913V3.16634C1.58337 2.74641 1.75019 2.34369 2.04712 2.04676C2.34405 1.74982 2.74678 1.58301 3.16671 1.58301H10.2917C10.7116 1.58301 11.1144 1.74982 11.4113 2.04676C11.7082 2.34369 11.875 2.74641 11.875 3.16634V3.95801M8.70837 7.12467H15.8334C16.7078 7.12467 17.4167 7.83356 17.4167 8.70801V15.833C17.4167 16.7075 16.7078 17.4163 15.8334 17.4163H8.70837C7.83392 17.4163 7.12504 16.7075 7.12504 15.833V8.70801C7.12504 7.83356 7.83392 7.12467 8.70837 7.12467Z" stroke="#1E1E1E" stroke-width="1.58333" stroke-linecap="round" stroke-linejoin="round"/>
                                    </svg>
                                </a>
                            </div>
                    ...
                    <td>
                        <?= $item->get_meta(__( 'Avbokningskydd')) ? 'Yes' : 'No' ?>
                    </td>
                    <td>
                        <?php
                        $deposit = (float)trim(strip_tags($item->get_meta(__( 'Deposition'))));
                        $dog = (float)trim(strip_tags($item->get_meta(__( 'Hund'))));
                        //$cancel = (float)trim(strip_tags($item->get_meta(__( 'Avbokningskydd'))));
                        $deposit_sum = $deposit + $dog;
                        ?>
                        <?= $deposit_sum ? wc_price($deposit_sum) : '-' ?>
                    </td>
                    <td>
                        <a href="javascript:;" class="btn send-guest_information_mail-btn" data-order_id="<?= $order->get_id() ?>" data-item_id="<?= $item->get_id() ?>">Send&nbsp;info</a>
                    </td>
                    <td>
                        <?php
                        $note = get_post_meta($order->get_id(), '_item_'.$item->get_id().'_table_note', true);
                        ?>
                        <div class="info-block" style="display: flex;justify-content: space-between;">
                            <button type="button" class="show-info-block" style="background-color: white; border: none;">
                                <?= $note ? 'Show' : '-' ?>
                            </button>
                            <div class="info-block-content" style="display: none">
                                <p><?= $note ?></p>
...
<script>
    (function($){
        ...
        $(document).on('click', '.orders-table th', function(){
            $('.orders-table th').not(this).removeClass('sort asc desc');
            $(this).addClass('sort');
            ...
            var rows = table.find('tr:gt(0)').toArray().sort(comparer(index, $(this).hasClass('asc')));
        ...
        function comparer(index, asc) {
            return function(a, b) {
                ...
                var isIgnoreA = valA === "ignore";
                var isIgnoreB = valB === "ignore";

                if (isIgnoreA && !isIgnoreB) return 1;
                if (!isIgnoreA && isIgnoreB) return -1;
                if (isIgnoreA && isIgnoreB) return 0;

                if (valA === valB) {
                    var priorityA = parseInt($(a).data('priority')) || 0;
                    var priorityB = parseInt($(b).data('priority')) || 0;

                    if (priorityA === priorityB) {
                        var extraA = getCellValue(a, 3);
                        var extraB = getCellValue(b, 3);

                        return ($.isNumeric(extraA) && $.isNumeric(extraB) ? extraA - extraB : extraA.toString().localeCompare(extraB)) * (asc ? 1 : -1);
                    }

                    return priorityA - priorityB;
                }

                return ($.isNumeric(valA) && $.isNumeric(valB) ? valA - valB : valA.toString().localeCompare(valB)) * (asc ? 1 : -1);
            };
        }

        function getCellValue(row, index){
            const $td = $(row).children('td').eq(index);
            return typeof $td.data('sort_value') !== 'undefined' ? $td.data('sort_value') : $td.text();
        }

        function showOrdersTableModal(content) {
            $('.orders-table-modal').remove();

            // Створюємо обгортку модального вікна
            let modal = $('<div class="orders-table-modal"></div>').css({
                'position': 'fixed',
                'top': '0',
                'left': '0',
                'width': '100%',
            ...
            let closeButton = $('<span class="close-btn"><svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M1.4 14L0 12.6L5.6 7L0 1.4L1.4 0L7 5.6L12.6 0L14 1.4L8.4 7L14 12.6L12.6 14L7 8.4L1.4 14Z" fill="#1D1B20"/> </svg></span>').css({
                'position': 'absolute',
                'top': '10px',
                'right': '15px',
                'cursor': 'pointer',
            ...
            // Додаємо контент та кнопку закриття
            modalContent.append(closeButton).append(content);
            modal.append(modalContent);
            $body.append(modal);
        }

        $body.on('click', '.show-info-block', function(){
            const $content = $(this).closest('.info-block').find('.info-block-content');
            showOrdersTableModal($content.html())
        });

        function copyToClipboard(content){
            let clean_content = content.replace(/^[\s]+/gm, '');
            var $temp = $('<textarea>');
            $('body').append($temp);
            $temp.val(clean_content).select();
        ...
        $body.on('click', '.orders-table .copy', function(){
            copyToClipboard($(this).closest('td').find('.info-block-content').text());
        })

        $body.on('click', '.modal-content .copy', function(){
            copyToClipboard($(this).closest('.modal-content').text());
        })


        $body.on('click', '.orders-table-filter button', function(){
            $(this).toggleClass('active btn-transparent');
            const filters = [];
            $('.orders-table-filter button.active').each(function (){
            ...
            $('.orders-table tbody tr').each(function(){
                ...
                if(filters.includes('cleaning') && $(this).data('cleaning') == 0){
                    isVisible = false;
                }

                if(filters.includes('past') && $(this).data('past') == 0){
                    isVisible = false;
                }
                else if(!filters.includes('past') && $(this).data('past') != 0){
                    isVisible = false;
                }
        ...
        $body.on('change', 'select.table_assign', function(){
            const order_id = $(this).data('order_id');
            const item_id = $(this).data('item_id');
            const _table_assign = $(this).val();
            $.ajax({
                type: "POST",
                url: "/wp-admin/admin-ajax.php",
                dataType: "json",
                data: {
                    action: 'set_order_item_table_assign',
                    order_id,
                    item_id,
                    _table_assign
                },
        ...
        $body.on('click', '.modal-content .edit', function(){
            const $wrap = $(this).closest('.modal-content');
            const content = $wrap.find('p').text();
            const textarea = $('<textarea class="textarea-to-save"></textarea>');
            textarea.val(content);
            $wrap.find('p').html(textarea);
            $wrap.find('.edit').hide();
            $wrap.find('.copy').hide();
            $wrap.find('.save').show();
        });

        $body.on('click', '.modal-content .save', function(){
            const $wrap = $(this).closest('.modal-content');
            const content = $wrap.find('.textarea-to-save').val();
            const order_id = $wrap.find('.save').data('order_id');
            const item_id = $wrap.find('.save').data('item_id');
            $wrap.find('p').text(content);
            const $td = $(`.orders-table .save[data-order_id="${order_id}"][data-item_id="${item_id}"]`).closest('td');
            $td.find('.info-block-content p').text(content);
            $td.find('button.show-info-block').text(content.length ? 'Show' : '-');
            $wrap.find('.save').hide();
            $wrap.find('.edit').show();
            $wrap.find('.copy').show();
            $.ajax({
                ...
                dataType: "json",
                data: {
                    action: 'set_order_item_table_note',
                    order_id,
                    item_id,
                    content
                },
        ...
        $(document).on('click', '.send-guest_information_mail-btn', function(){
            if(confirm('Are you sure?')){
                $.ajax({
                    ...
                    dataType: "json",
                    data: {
                        action: 'send_guest_information_mail',
                        order_id: $(this).data('order_id'),
                        order_item_id: $(this).data('item_id')
                    },
                    beforeSend: function () { },
                    complete: function () {},
                    success: function (json) {
        ...
        $(document).ready(function(){
            $body.find('.orders-table').find('thead th').first().click();
            $body.find('.orders-table').show();
           /* setTimeout(function(){
                $body.find('.orders-table').find('thead th').first().click();
                $body.find('.orders-table').show();
            }, 200)*/
